...
class Initial_LadderBlock(nn.Module):
    ...
    def __init__(self, planes, layers, kernel=3, block=BasicBlock, inplanes=3):
        ...
        for i in range(0, layers):
            ...
            self.attn_list.append(Attention_Block(F_g=planes * 2 ** (layers - i - 1), F_l=planes * 2 ** (layers - i - 1),
                                                  F_int=planes * 2 ** max(0, layers - i - 1)))
    ...
    def forward(self, x):
        ...
        for j in range(0, self.layers):
            ...
            attn = self.attn_list[j](g=out, x=down_out[self.layers - j - 1])
            out = out + attn
            # out = F.relu(out)
